...
class SWApi {
    ...
    static async getStatistics() {
        ...
        await Promise.all(postNames.map(async name => {
    ...
    static async _getLimitedPaginatedPosts({ postsName, postsPerPage, postsCount, startPost, limit }) {
        const end = startPost + limit - 1;
        const startPage = ((startPost - 1) / postsPerPage | 0) + 1;
        const endPage = ((end - 1) / postsPerPage | 0) + 1;
        if (startPost > postsCount)
            return [];
